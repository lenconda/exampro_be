...
export interface RedisClient {
  defaultKey: string;
  clients: Map<string, redis.RedisClient>;
  size: number;
}
...
async function getClient(
  ...
  const { onClientReady, url, ...restOptions } = options;
  const client = url
    ? redis.createClient(url)
    : redis.createClient(restOptions);
  if (onClientReady) {
...
export const createClient = (): Provider => ({
  ...
  useFactory: async (
    ...
    const clients = new Map<string, redis.RedisClient>();
